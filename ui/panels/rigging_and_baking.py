...
class BLENRIG_PT_Rigging_and_baking(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # to show this in place we have to have it duplicated, hide one and then show the other, I know the code is duplicated, but
        # it's the only way I can think of to get the extras at the bottom always.
        if context.mode in ['POSE','OBJECT']:
            ...
            elif context.active_object.data.reproportion:
                box = col.box()
                row = box.row()
                row.scale_x = 0.5
                row.scale_y = 1.8
