        ...
        if (context.active_object.type in ["MESH"]):
            if hasattr(context.active_object, 'modifiers'):
                for mod in context.active_object.modifiers:
                    if mod.type == 'MESH_DEFORM' and mod.is_bound == False:
                        return True
        else:
            return False
...
class Operator_blenrig_guide_bind_mdef_modifiers(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.blenrig.guide_transfer_test_rig(bone="master_torso")
        return {"FINISHED"}
...
class Operator_blenrig_unbind_mdef_modifiers(bpy.types.Operator):
    ...
    def poll(cls, context):
            ...
            if (context.active_object.type in ["MESH"]):
                if hasattr(context.active_object, 'modifiers'):
                    for mod in context.active_object.modifiers:
                        if mod.type == 'MESH_DEFORM' and mod.is_bound == True:
                            return True
            else:
                return False
...
class Operator_blenrig_guide_edit_mdef_cage(bpy.types.Operator):
    ...
    def execute(self, context):
        guide_props = bpy.context.scene.blenrig_guide
        char_body = bpy.context.scene.blenrig_character_body_obj[0]
        ...
        #Unbind Mdef
        set_active_object(context, char_body['character_body_obj'])
